...
const std::vector<D3D11_INPUT_ELEMENT_DESC> TVOFEffec::INPUT_LAYOUT =
{
...
void TVOFEffec::LoadTexture(const wchar_t* path)
{
	//		画像ファイルの読み込み
...
void TVOFEffec::Create(const wchar_t* path,
...
void TVOFEffec::LoadShader()
{
	auto device = LibrarySingleton::GetInstance()->GetDeviceResources()->GetD3DDevice();

	//		コンパイルされたシェーダファイルを読み込み
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shader/TVOFF/TVOFFVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shader/TVOFF/TVOFFGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shader/TVOFF/TVOFFPS.cso");
...
void TVOFEffec::Render(float time)
{
