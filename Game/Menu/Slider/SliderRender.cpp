...
const std::vector<D3D11_INPUT_ELEMENT_DESC> SliderRender::INPUT_LAYOUT =
{
...
void SliderRender::LoadTexture(const wchar_t* path)
{
	//		画像ファイルの読み込み
...
void SliderRender::Create(const wchar_t* path,
...
void SliderRender::LoadShader()
{
	auto device = LibrarySingleton::GetInstance()->GetDeviceResources()->GetD3DDevice();

	//		コンパイルされたシェーダファイルを読み込み
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shader/Slider/SliderVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shader/Slider/SliderGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shader/Slider/SliderPS.cso");
...
void SliderRender::Render(float time)
{
