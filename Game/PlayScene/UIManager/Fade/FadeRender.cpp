...
const std::vector<D3D11_INPUT_ELEMENT_DESC> FadeRender::INPUT_LAYOUT =
{
...
void FadeRender::LoadTexture(const wchar_t* path)
{
	//		画像ファイルの読み込み
...
void FadeRender::Create(const wchar_t* path,
...
void FadeRender::LoadShader()
{
	auto device = LibrarySingleton::GetInstance()->GetDeviceResources()->GetD3DDevice();

	//		コンパイルされたシェーダファイルを読み込み
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shader/Fade/FadeShaderVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shader/Fade/FadeShaderGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shader/Fade/FadeShaderPS.cso");
...
void FadeRender::Render(float time)
{
