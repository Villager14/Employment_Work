...
const std::vector<D3D11_INPUT_ELEMENT_DESC> NumberShader::INPUT_LAYOUT =
{
...
void NumberShader::LoadTexture(const wchar_t* path)
{
	//		画像ファイルの読み込み
...
void NumberShader::Create(const wchar_t* path,
...
void NumberShader::LoadShader()
{
	auto device = LibrarySingleton::GetInstance()->GetDeviceResources()->GetD3DDevice();

	//		コンパイルされたシェーダファイルを読み込み
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shader/Number/NumberVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shader/Number/NumberGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shader/Number/NumberPS.cso");
...
void NumberShader::Render(int number)
{
