...
const std::vector<D3D11_INPUT_ELEMENT_DESC> UIRender::INPUT_LAYOUT =
{
...
void UIRender::LoadTexture(const wchar_t* path)
{
	//		画像ファイルの読み込み
...
void UIRender::Create(const wchar_t* path,
...
void UIRender::LoadShader()
{
	auto device = LibrarySingleton::GetInstance()->GetDeviceResources()->GetD3DDevice();

	//		コンパイルされたシェーダファイルを読み込み
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shader/UI/UIShaderVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shader/UI/UIShaderGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shader/UI/UIShaderPS.cso");
