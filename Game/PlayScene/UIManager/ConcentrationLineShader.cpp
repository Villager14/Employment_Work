/*
* @file		ConcentrationLineShader.cpp
* @brief	集中線の描画
* @author	Morita
* @date		2024/06/17
*/
...
const std::vector<D3D11_INPUT_ELEMENT_DESC> ConcentrationLineShader::INPUT_LAYOUT =
{
...
ConcentrationLineShader::ConcentrationLineShader()
{
}
...
void ConcentrationLineShader::LoadTexture(const wchar_t* path)
{
	//		画像ファイルの読み込み
...
void ConcentrationLineShader::Create(const wchar_t* path,
...
void ConcentrationLineShader::LoadShader()
{
	auto device = LibrarySingleton::GetInstance()->GetDeviceResources()->GetD3DDevice();

	//		コンパイルされたシェーダファイルを読み込み
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shader/ConcentrationLine/ConcentrationLineVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shader/ConcentrationLine/ConcentrationLineGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shader/ConcentrationLine/ConcentrationLinePS.cso");
...
void ConcentrationLineShader::Render(float move)
{
	...
	cbuff.concentrationLineTime = { move, 0.0f, 0.0f, 0.0f };
