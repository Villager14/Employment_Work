...
const std::vector<D3D11_INPUT_ELEMENT_DESC> CircleShader::INPUT_LAYOUT =
{
...
void CircleShader::LoadTexture(const wchar_t* path)
{
	//		画像ファイルの読み込み
...
void CircleShader::Create(const wchar_t* path,
...
void CircleShader::LoadShader()
{
	...
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shader/CoolTime/CoolTimeVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shader/CoolTime/CoolTimeGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shader/CoolTime/CoolTimePS.cso");
...
void CircleShader::Render(int number)
{
	...
	cbuff.rotation = { static_cast<float>(number),0,0,0 };
