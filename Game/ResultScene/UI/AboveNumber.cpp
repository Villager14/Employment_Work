...
#include "AboveNumber.h"

#include <WICTextureLoader.h>
#include "Game/PlayScene/UIManager/BinaryFile.h"

const std::vector<D3D11_INPUT_ELEMENT_DESC> AboveNumber::INPUT_LAYOUT =
{
...
void AboveNumber::LoadTexture(const wchar_t* path)
{
	//		画像ファイルの読み込み
...
void AboveNumber::Create(const wchar_t* path,
...
void AboveNumber::LoadShader()
{
	auto device = LibrarySingleton::GetInstance()->GetDeviceResources()->GetD3DDevice();

	//		コンパイルされたシェーダファイルを読み込み
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shader/AboveNumber/AboveNumberVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shader/AboveNumber/AboveNumberGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shader/AboveNumber/AboveNumberPS.cso");
...
void AboveNumber::Render(int number, float time)
{
	...
	cbuff.number = { static_cast<float>(number),0,0,0 };
	cbuff.time = { time, 0,0,0 };
